...
Openfort transactions are executed from a smart contract wallet.
Transactions, also called "transaction intents", are the best way to track the status and details of each one.
...
Transactions are executed by sending an authenticated transaction request to the Openfort API.
