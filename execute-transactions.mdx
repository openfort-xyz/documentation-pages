...
However, if the API call was performed by a player using a non-custodial account, the response of the call above would contain a `next_action` parameter explained above.
That parameter indicates the information that the player needs to sign to approve the transaction intent.
...
<Note>
  ...
  <br /> In order to execute the transaction intents above, the player should have native tokens in the account to pay for transaction fees.
  If you want to be able to sponsor transaction gas or allow players to pay them in ERC20, you can use a paymaster.
  <br /> Visit [Gas Payments](/gas-payments) for more information.
</Note>
