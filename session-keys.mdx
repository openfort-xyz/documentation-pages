...
<CodeGroup title="Regsiter the session key using Openfort" label={'node.js'} label="/v1/sessions" tag="POST">
  ```bash {{ title: 'cURL' }}
...
it has to be sent using the endpoint `/v1/sessions/:id/signature` as shown below:
...
Therefore, from the request above, the session key will need to sign the next_action `user_op_hash` and send it to the API.
