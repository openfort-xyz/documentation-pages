...
![SessionKeys.excalidraw.svg](https://blog-cms.openfort.xyz/uploads/How_session_keys_work_1_772605d591.svg)

## How Session Keys Work
When players log into the game, they can generate a session key for that device. The session key is then securely notified to the game server and is registered in the player's web3 account (aka smart contract wallet). Subsequent requests from the player's device to the game use the specific game's authentication mechanism of each game together with the user's session key. Openfort's infrastructure validates the session key and verifies the player's identity before processing the request.
