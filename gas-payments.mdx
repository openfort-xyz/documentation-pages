...
<CodeGroup
  title="List all allowed functions for a policy"
  tag="GET"
  label="/v1/allow_functions"
>
  ```bash {{ title: 'cURL' }}
  curl -G https://api.openfort.xyz/v1/allow_functions \ -H "Authorization: Bearer
  $YOUR_SECRET_KEY" \ -d policy=pol_... ```
</CodeGroup>
