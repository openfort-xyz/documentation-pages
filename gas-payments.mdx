...
When selecting `Allowed transactions` as the rule model, you can decide what asset contract you want the gas strategy to be applied.
...
When selecting `Register sessions` as the rule model, the affected policy will allow the registration of new session keys. 
...
You can also use the Openfort API to directly interact with gas policies. Here is an example:
