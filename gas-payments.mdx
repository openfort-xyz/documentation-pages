...
<CodeGroup title="Create a policy for gas sponsoring" tag="POST" label="/v1/policies">
  ```bash {{ title: 'cURL' }}
    curl https://api.openfort.xyz/v1/policies/ \
      -H "Authorization: Bearer $YOUR_SECRET_KEY" \
      -d chain_id=5 \
      -d name="TEST" \
      -d "strategy[sponsor_schema]=pay_for_user" \
      -d "strategy[token_contract]=" \
      -d "strategy[token_contract_amount]="
  ```
</CodeGroup>
