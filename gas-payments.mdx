...
- When selecting `Allowed transactions` as the policy rule, you can decide what asset contract and functions are allowed to be called by your users.
...
<CodeGroup title="Create a policy rules for the policy" tag="POST" label="/v1/policy_rules">
  ```bash {{ title: 'cURL' }}
